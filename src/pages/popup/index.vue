<template>
  <demo-section class="demo-popup">
    <demo-block card :title="t('basicUsage')">
      <van-cell :title="t('buttonBasic')" is-link @click="showBasic = true" />
      <van-popup v-model="showBasic" :style="{ padding: '30px 50px' }" duration="0.3">
        {{ t('content') }}
      </van-popup>
    </demo-block>
    ...
    <demo-block card :title="t('position')">
      ...
      <van-popup v-model="showTop" position="top" :style="{ height: '30%' }" duration="0.3" />
      <van-popup
        v-model="showBottom"
        position="bottom"
        :style="{ height: '30%' }"
